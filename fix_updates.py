...
# We need to manually split the content into colleges
...
def replace_college(m):
    ...
    if col_id in updates:
        ...
        # Split into branches
        parts = re.split(r'(\{\s*branch:\s*"[^"]+".*?(?=\s*\},|\s*\}$))', branches_str, flags=re.DOTALL)
        
        # In this parts array, we need to carefully find the branch and replace
        # Actually, let's just use re.sub for each branch independently over branches_str!
        
        for branch_name, quotas in branch_updates.items():
            def branch_replacer(bm):
                branch_block = bm.group(0)
                for quota, (min_val, max_val) in quotas.items():
                    # Replace quota inside this specific branch_block
                    q_pat = r'({0}:\s*\{{\s*minRank:\s*)\d+(,\s*maxRank:\s*)\d+(\s*\}})'.format(quota)
                    branch_block = re.sub(q_pat, r'\g<1>{0}\g<2>{1}\g<3>'.format(min_val, max_val), branch_block)
                return branch_block

            # The pattern for a branch block that EXACTLY matches the branch_name
            # branch: "branch_name", ... up to the next } 
            bp = r'\{\s*branch:\s*"' + re.escape(branch_name) + r'".*?(?=\s*\},|\s*\}$)'
            branches_str = re.sub(bp, branch_replacer, branches_str, flags=re.DOTALL)
            
    return prefix + branches_str + suffix

# we must use a while loop or re.sub to process all matches
new_content = college_pattern.sub(replace_college, content)

with open(FILE_PATH, 'w', encoding='utf-8') as f:
    f.write(new_content)

print("Remaining updates successfully written! Properly!")
